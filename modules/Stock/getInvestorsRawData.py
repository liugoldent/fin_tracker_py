...
import requests
import sys
sys.path.append('/Users/guantingliu/Desktop/BackEnd/fin_tracker_py/public')
from lxml import html, etree
from fakeUserAgentGenerate import userAgentRoute
import json

# 爬蟲：三大法人買賣超資料，爬完的純資料
def getInvestorsRawData(type):
    response = requests.get(typeSelect(type), headers=  {'User-Agent': userAgentRoute()})
    htmlTree = etree.HTML(response.text)

    date = htmlTree.xpath('//div[@class="t11"]/text()')
    buyRawCodeNameList = htmlTree.xpath('//table[@class="t01"]/tr/td[2]/a/text()')
    buyRawCodeList = [a.attrib['href'] for a in htmlTree.xpath('//table[@class="t01"]/tr/td[2]/a')]
    sellRawCodeNameList = htmlTree.xpath('//table[@class="t01"]/tr/td[7]/a/text()')
    sellRawCodeList = [a.attrib['href'] for a in htmlTree.xpath('//table[@class="t01"]/tr/td[7]/a')]
    
    buyList = dealRawCrawData(buyRawCodeNameList, buyRawCodeList, 'buy', date)
    sellList = dealRawCrawData(sellRawCodeNameList, sellRawCodeList, 'sell', date)
